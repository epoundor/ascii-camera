...
var ascii = (function () {
  function asciiFromCanvas(canvas: HTMLCanvasElement, options: Option) {
    ...
    for (var y = 0; y < canvasHeight; y += 2) {
      ...
      for (var x = 0; x < canvasWidth; x++) {
        ...
        var color = getColorAtOffset(imageData ? imageData.data : null, offset);
  ...
  function getColorAtOffset(data: Uint8ClampedArray | null, offset: number) {
    if (data)
      return {
        red: data[offset],
        green: data[offset + 1],
  ...
  return {
    fromCanvas: function (canvas: HTMLCanvasElement, options: Option) {
      options = options;
      options.contrast =
